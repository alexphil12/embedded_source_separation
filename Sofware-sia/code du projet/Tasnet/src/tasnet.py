...
class Separator(nn.Module):
    ...
    def __init__(self, N, hidden_size, num_layers, bidirectional=False, nspk=2):
        ...
        self.rnn = nn.LSTM(input_size=N, hidden_size=hidden_size, num_layers=num_layers,
                           batch_first=True,
                           dropout=0,
    ...
    def forward(self, mixture_w, mixture_lengths):
        ...
        mixture_lengths=mixture_lengths.tolist()
        packed_input = pack_padded_sequence(norm_mixture_w,mixture_lengths,True,True)
...
if __name__ == "__main__":
    ...
    mixture = torch.randint(3, (B, K, L)).float()
    lengths = torch.LongTensor([K for i in range(B)])
    # test Encoder
    encoder = Encoder(L, N)
    encoder.conv1d_U.weight.data = torch.randint(2, encoder.conv1d_U.weight.size()).float()
    encoder.conv1d_V.weight.data = torch.randint(2, encoder.conv1d_V.weight.size()).float()
